        ...
        public HttpPathHandler this[string path]
        {
            ...
            set
            {
                lock (_paths)
                {
                    ...
                    if (value == null)
                    {
                        CoreManager.ServerCore.ConsoleManager.Debug("Web Admin", CoreManager.ServerCore.StringLocale.GetString("CORE:DEBUG_WEBADMIN_HANDLER_REMOVED",  path));
                        return;
                    }
                    _paths.Add(path, value);
                    if(newEntry)
                        CoreManager.ServerCore.ConsoleManager.Debug("Web Admin",CoreManager.ServerCore.StringLocale.GetString("CORE:DEBUG_WEBADMIN_HANDLER_ADDED", path));
                    else
                        CoreManager.ServerCore.ConsoleManager.Debug("Web Admin", CoreManager.ServerCore.StringLocale.GetString("CORE:DEBUG_WEBADMIN_HANDLER_CHANGED", path));
                }
            }
        }
        #endregion
